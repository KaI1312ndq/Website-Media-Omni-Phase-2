...
const Auth = {
  get() {
    try {
      const s = sessionStorage.getItem('mo_user');
      if (s) return JSON.parse(s);
      // Fallback về localStorage để giữ phiên sau khi refresh / mở tab mới
      const l = localStorage.getItem('mo_persist');
      if (l) { sessionStorage.setItem('mo_user', l); return JSON.parse(l); }
      return null;
    } catch { return null; }
  },
  set(u) {
    ...
    localStorage.setItem('mo_persist', j); // lưu dài hạn
  },
  clear() {
    sessionStorage.removeItem('mo_user');
    localStorage.removeItem('mo_persist');
  },
