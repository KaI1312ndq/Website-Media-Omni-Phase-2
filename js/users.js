...
async function loadUsers(){
  ...
  }catch{document.getElementById('user-tbody').innerHTML='<div class="no-users">❌ Không tải được dữ liệu.</div>';}
}
...
function renderUserList(){
  ...
  body.innerHTML=filtered.map(u=>{
    ...
    return `<div class="ut-row">
      <div><div class="ut-name">${u.name||'—'}</div></div>
      <div class="ut-user">${u.username}</div>
      <span><span class="role-pill ${rCls}">${u.role}</span></span>
      <span><span class="status-pill ${sCls}">${u.status==='disabled'?'Disabled':'Active'}</span></span>
      <span style="font-size:.78rem;color:var(--faint)">${created}</span>
      <div class="ut-actions">
        <button class="ua-btn" onclick="openEdit('${u.username}')">Sửa</button>
        <button class="ua-btn" onclick="toggleStatus('${u.username}','${u.status}')">${u.status==='disabled'?'Bật':'Tắt'}</button>
        ${!isMe?`<button class="ua-btn ua-del" onclick="deleteUser('${u.username}')">Xóa</button>`:''}
      </div>
    </div>`;
  }).join('');
}
...
function openEdit(username){
  ...
  document.getElementById('f-pass').placeholder='Để trống = giữ nguyên mật khẩu';
...
async function toggleStatus(username,cur){
  ...
  try{
    ...
    showToast('✅ Đã cập nhật');loadUsers();
...
async function deleteUser(username){
  ...
  try{
    ...
    showToast('✅ Đã xóa');loadUsers();
  }catch{showToast('❌ Lỗi','error');}
}

async function changeMyPass(){
  const oldp=document.getElementById('my-oldpass').value.trim();
  const newp=document.getElementById('my-newpass').value.trim();
  const cfmp=document.getElementById('my-cfmpass').value.trim();
  if(!oldp){showToast('Vui lòng nhập mật khẩu cũ','error');return;}
  if(!newp||newp.length<6){showToast('Mật khẩu mới tối thiểu 6 ký tự','error');return;}
  if(newp!==cfmp){showToast('Mật khẩu xác nhận không khớp','error');return;}
  const me=Auth.get();
  // Verify old password qua API
  try{
    const vf=await fetch(USERS_API,{method:'POST',body:JSON.stringify({type:'user',action:'verify',username:me.username,password:oldp})}).then(r=>r.json());
    if(vf.status!=='ok'){showToast('Mật khẩu cũ không đúng','error');return;}
    await fetch(USERS_API,{method:'POST',body:JSON.stringify({type:'user',action:'update',username:me.username,password:newp})});
    showToast('✅ Đã đổi mật khẩu thành công!');
    ['my-oldpass','my-newpass','my-cfmpass'].forEach(id=>document.getElementById(id).value='');
  }catch{showToast('❌ Lỗi','error');}
}
