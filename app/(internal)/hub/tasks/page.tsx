...
export default function TasksPage() {
  ...
  useEffect(() => {
    ...
    if (u) {
      setUser(u)
      setForm(f => ({ ...f, assignee: u.username }))
    }
    fetch('/api/users')
      .then(r => r.json())
      .then(({ users }) => {
  ...
  const loadTeamCal = useCallback(async () => {
    ...
    const ws = fmt(dates[0])
    const we = fmt(dates[4])
  ...
  useEffect(() => {
    if (user && tab === 'myday') loadMyDay(curDate)
  }, [user, tab, curDate, loadMyDay])
  useEffect(() => {
    if (user && tab === 'team') loadTeamCal()
  }, [user, tab, weekOff, loadTeamCal])
  ...
  async function createTask() {
    ...
    if (!form.assignee) {
      showToast('Chọn người được assign', 'error')
      return
    }
    setCreating(true)
    ...
    await fetch('/api/tasks', {
      ...
      body: JSON.stringify({
        ...form,
        assignee_name: assigneeMember?.name,
  ...
  return (
    <>
        ...
        <div className="popup-overlay on" onClick={() => setPopup(null)}>
          <div className="popup-card" onClick={e => e.stopPropagation()}>
            ...
            <div className="popup-row">
              <span className="popup-lbl">Assignee</span>
              <span className="popup-val">{popup.assignee_name || popup.assignee}</span>
            </div>
            <div className="popup-row">
              <span className="popup-lbl">Thời gian</span>
              <span className="popup-val">
                {popup.time_start
                  ? `${fmtTime(popup.time_start)}${popup.time_end ? ' – ' + fmtTime(popup.time_end) : ''}`
                  : 'Không có giờ'}
              </span>
            </div>
            <div className="popup-row">
              <span className="popup-lbl">Ưu tiên</span>
              <span className="popup-val">{PRI_LABEL[popup.priority]}</span>
            </div>
            <div className="popup-row">
              <span className="popup-lbl">Tạo bởi</span>
              <span className="popup-val">{popup.created_name || '—'}</span>
            </div>
            {popup.description && (
              <div className="popup-row">
                <span className="popup-lbl">Mô tả</span>
                <span className="popup-val">{popup.description}</span>
              </div>
            )}
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 18, flexWrap: 'wrap', paddingTop: 16 }}>
        <button
          className={`btn-${tab === 'myday' ? 'p' : 's'}`}
          onClick={() => setTab('myday')}
          style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}
        >
          <span style={{ display: 'inline-flex' }}>{Icon.calendar(14)}</span>My Day
        </button>
        <button
          className={`btn-${tab === 'team' ? 'p' : 's'}`}
          onClick={() => setTab('team')}
          style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}
        >
          <span style={{ display: 'inline-flex' }}>{Icon.users(14)}</span>Team
        </button>
        {(user?.role === 'admin' || user?.perms?.tasks_create) && (
          <button
            className={`btn-${tab === 'create' ? 'p' : 's'}`}
            onClick={() => setTab('create')}
            style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}
          >
            <span style={{ display: 'inline-flex' }}>{Icon.plus(14)}</span>Tạo task
          </button>
        )}
      </div>
      ...
      {tab === 'myday' && (
        <div className="rw" style={{ maxWidth: 1100, margin: '0 auto', padding: '0 0 40px' }}>
          <div className="date-bar">
            <h1 style={{ fontFamily: 'var(--f-display)', fontSize: '1.6rem', fontWeight: 800 }}>
              My Day — <span>{DAYS[curDate.getDay()]}</span>
            </h1>
            <div className="date-nav">
              ...
              <span className="date-label">
                {fmtShort(curDate)} / {curDate.getFullYear()}
              </span>
              <button
                className="date-btn"
                onClick={() =>
              ...
              <button className="date-today-btn" onClick={() => setCurDate(new Date())}>
                Hôm nay
              </button>
            </div>
          </div>
              ...
              <div className="tl-body">
                {loading ? (
                  ...
                  <div className="t-empty">
                    <div className="t-empty-ico" style={{ display: 'inline-flex', color: '#475569' }}>
                      {Icon.inbox(36)}
                    </div>
                    <div className="t-empty-tt">Không có task nào</div>
                    <div className="t-empty-sb">Hôm nay chưa có task được assign cho bạn.</div>
                  </div>
                ) : (
                  (() => {
                    const hours = [
                      ...
                      <>
                        {noTime.length > 0 && (
                          <>
                            ...
                            {noTime.map(t => (
                              <TaskCard key={t.id} t={t} onToggle={toggleTask} onDetail={setPopup} />
                            ))}
                            ...
                            <div key={h} className="tl-slot">
                              ...
                              <div className="tl-tasks">
                                {slot.length ? (
                                  ...
                                  <div className="tl-empty">Trống</div>
                                )}
                              </div>
                            </div>
                          )
                ...
                <div className="prog-ring">
                  <div className="pr-wrap">
                    ...
                    <div className="pr-txt">
                      <div className="pr-pct">{pct}%</div>
                      <div className="pr-lbl">Hoàn thành</div>
                    </div>
                  </div>
                </div>
                <div className="stat-2">
                  <div className="stat-cell">
                    <div className="stat-v">{done}</div>
                    <div className="stat-l">Xong</div>
                  </div>
                  <div className="stat-cell">
                    <div className="stat-v">{todayTasks.length - done}</div>
                    <div className="stat-l">Còn lại</div>
                  </div>
                </div>
              </div>
              <div className="t-scard">
                ...
                {(() => {
                  ...
                  const upcoming = todayTasks
                    .filter(t => t.status !== 'done' && t.time_start && fmtTime(t.time_start) >= nowStr)
                    .slice(0, 5)
      ...
      {tab === 'team' && (
        <div className="rw" style={{ maxWidth: 1200, margin: '0 auto', padding: '0 0 40px' }}>
          <div className="date-bar">
            <h1 style={{ fontFamily: 'var(--f-display)', fontSize: '1.6rem', fontWeight: 800 }}>
              Team <span>Calendar</span>
            </h1>
            <div className="date-nav">
              <button className="date-btn" onClick={() => setWeekOff(w => w - 1)}>
                ←
              </button>
              <span className="date-label">
                {weekDates.length ? `${fmtShort(weekDates[0])} – ${fmtShort(weekDates[4])}` : 'Tuần này'}
              </span>
              <button className="date-btn" onClick={() => setWeekOff(w => w + 1)}>
                →
              </button>
              <button className="date-today-btn" onClick={() => setWeekOff(0)}>
                Tuần này
              </button>
            </div>
          </div>
          {loading ? (
            ...
            <div className="cal-wrap" style={{ overflowX: 'auto' }}>
              <div
                className="cal-row-hdr"
                style={{ gridTemplateColumns: '150px repeat(5,1fr)', display: 'grid', gap: 1 }}
              >
                ...
                {weekDates.map((d, i) => {
                  ...
                  return (
                    <div
                      key={i}
                      className="cal-hdr-cell"
                      style={{ ...(isToday ? { color: 'var(--blue)', background: 'var(--border2)' } : {}) }}
                    >
                      {['T2', 'T3', 'T4', 'T5', 'T6'][i]}
                      <br />
                      <span style={{ fontSize: '.8em', opacity: 0.7 }}>{fmtShort(d)}</span>
                    </div>
                  )
                })}
              </div>
                    ...
                    return (
                      <div key={i} className="cal-cell">
                        {dt.slice(0, 3).map(t => (
                          <div
                            key={t.id}
                            className={`cal-task-mini ctm-${t.priority[0]}`}
                            onClick={() => setPopup(t)}
                            title={t.task_name}
                          >
                            {t.time_start ? fmtTime(t.time_start) + ' ' : ''}
                            {t.task_name}
                          </div>
                        ))}
                        {dt.length > 3 && (
                          <div style={{ fontSize: '.62rem', color: 'var(--faint)', padding: '.1rem .35rem' }}>
                            +{dt.length - 3}
                          </div>
                        )}
                      </div>
      ...
      {tab === 'create' && (
        <div className="rw" style={{ maxWidth: 680, margin: '0 auto', padding: '0 0 40px' }}>
          <div className="rc">
            <h2
              style={{ fontFamily: 'var(--f-display)', fontSize: '1.3rem', fontWeight: 800, marginBottom: 4 }}
            >
              Tạo task mới
            </h2>
            <p style={{ fontSize: '.88rem', color: 'var(--muted)', marginBottom: 20 }}>
              Assign task cho bản thân hoặc thành viên khác trong team.
            </p>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              <div style={{ gridColumn: '1/-1' }}>
                ...
                <input
                  ...
                  placeholder="VD: Review creative TikTok Shop..."
                />
              </div>
              <div style={{ gridColumn: '1/-1' }}>
            ...
            <div className="btn-row" style={{ marginTop: 20 }}>
              <button className="btn-s" onClick={() => setTab('myday')}>
                Huỷ
              </button>
...
function TaskCard({
  ...
  return (
    <div
      className={`task-card ${CARD_CLS[t.priority]} ${t.status === 'done' ? 'done' : ''}`}
      onClick={() => onDetail(t)}
    >
      <div className="tc-top">
        <div
          className={`tc-check ${t.status === 'done' ? 'checked' : ''}`}
          onClick={e => {
            e.stopPropagation()
            onToggle(t.id, t.status === 'done')
          }}
        />
      ...
      <div className="tc-meta">
        {t.time_start && (
          <span className="tc-time">
            ⏰ {fmtTime(t.time_start)}
            {t.time_end ? ' – ' + fmtTime(t.time_end) : ''}
          </span>
        )}
