...
const STATUS_LABEL: Record<LeadStatus, string> = {
  new: 'New', contacted: 'Đã liên hệ', qualified: 'Qualified', closed: 'Closed',
}

const CHANNEL_COLOR: Record<string, string> = {
  shopee: 'lc-shopee', tiktok: 'lc-tiktok', meta: 'lc-meta', google: 'lc-google', livestream: 'lc-live',
}
...
export default function AdminLeadsPage() {
  ...
  useEffect(() => { if (me) load() }, [me, filter, fromDate, toDate]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  function exportCsv() {
    const headers = ['Date', 'Name', 'Email', 'Phone', 'Brand', 'Channels', 'Budget', 'Status', 'Note', 'UTM Source', 'UTM Medium', 'UTM Campaign']
    const rows = filtered.map(l => [
      new Date(l.created_at).toLocaleString('vi-VN'),
      l.name, l.email, l.phone || '', l.brand || '',
      (l.channels || []).join(';'), l.monthly_budget || '',
      l.status, (l.note || '').replace(/[\r\n]+/g, ' '),
  ...
  return (
    <div className="leads-page">
      ...
      <div className="leads-filter">
        <select value={filter} onChange={e => { setFilter(e.target.value as LeadStatus | ''); setPage(1) }}>
        ...
        <input
          type="search" placeholder="Tìm tên / email / brand"
          value={q}
          onChange={e => setQ(e.target.value)}
          onKeyDown={e => { if (e.key === 'Enter') load() }}
        />
        <button className="lp-btn" onClick={load}>Refresh</button>
        <button className="lp-btn lp-btn-primary" onClick={exportCsv} disabled={filtered.length === 0}>Export CSV</button>
      </div>
            ...
            <tbody>
              {pageRows.map(l => (
                <Fragment key={l.id}>
                  ...
                  >
                    <td className="lp-date">{new Date(l.created_at).toLocaleDateString('vi-VN')}<br /><span>{new Date(l.created_at).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })}</span></td>
                    ...
                    <td>
                      <div className="lp-chans">
                        {(l.channels || []).map(c => (
                          <span key={c} className={`lp-chan ${CHANNEL_COLOR[c] || ''}`}>{c}</span>
                        ))}
                      </div>
                    </td>
                    ...
                    <td className="lp-note-preview">{l.note ? (l.note.length > 40 ? l.note.slice(0, 40) + '…' : l.note) : '-'}</td>
                    <td onClick={e => e.stopPropagation()}>
                      <div className="lp-actions">
                        {l.status === 'new' && (
                          <button className="lp-act" title="Đã liên hệ" onClick={() => updateStatus(l.id, 'contacted')}>
                            {Icon.check(14)}
                          </button>
                        )}
                    ...
                    <tr className="lp-expand-row">
                      <td colSpan={9}>
                        <div className="lp-expand">
                          <div><strong>Note đầy đủ:</strong> {l.note || '-'}</div>
                          <div><strong>UTM:</strong> {l.utm_source || '-'} · {l.utm_medium || '-'} · {l.utm_campaign || '-'}</div>
                          <div><strong>IP:</strong> {l.ip_address || '-'}</div>
                          <div><strong>User-Agent:</strong> <span className="lp-ua">{l.user_agent || '-'}</span></div>
                          <div><strong>Timeline:</strong> tạo {new Date(l.created_at).toLocaleString('vi-VN')}{l.contacted_at ? ` · liên hệ ${new Date(l.contacted_at).toLocaleString('vi-VN')} bởi ${l.contacted_by || '-'}` : ''}</div>
                        </div>
                      </td>
                    </tr>
                  )}
