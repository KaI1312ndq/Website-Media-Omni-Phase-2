...
const TITLES: Record<string, { greeting: string; subline: string }> = {
  ...
  '/hub/analytics': { greeting: 'Analytics — Operations', subline: 'Plan completeness, report coverage, data consistency & report timing.' },
  '/hub/scores':    { greeting: 'Bảng điểm Team', subline: 'Tổng hợp điểm quiz toàn team. Filter theo quiz, ngày, user.' },
...
export default function InternalRouteLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!user) {
    return <div className="loadingScreen" style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--f-mono)', color: 'var(--faint)' }}>Đang tải...</div>
  }

  const firstName = user.name.split(' ').pop() || user.name
  const cfg = TITLES[pathname] ?? { greeting: `Xin chào, ${firstName} 👋`, subline: 'Chào mừng bạn quay trở lại hệ thống Media Omni.' }
